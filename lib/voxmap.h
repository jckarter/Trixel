...
void voxmap_fill(voxmap * v, struct int3 low, struct int3 high, unsigned char fill);
void voxmap_fill_all(voxmap * v, unsigned char fill);

voxmap * voxmap_maskify(voxmap * v, unsigned char mask_fill);
voxmap * voxmap_normify(voxmap * v, unsigned char mask_fill[4]);
void voxmap_add(voxmap * to, voxmap const * from);
void voxmap_sub(voxmap * to, voxmap const * from);

int voxmap_count(voxmap * v);

static inline unsigned char * voxmap_voxel(voxmap * v, int x, int y, int z)
